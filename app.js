...
const MONGO_DB_URL = 
  `mongodb+srv://${process.env.MONGO_DB_USERNAME}:${process.env.MONGO_DB_PASSWORD}@cluster0.ndjshel.mongodb.net/${process.env.MONGO_DB_DATABASE}`;
...
const fileFilter = (req,file,cb) =>{
  ...
  const logginPath = path.join(rootDir,'access.log');
  const logginStream = fs.createWriteStream(logginPath, {flag: 'a'});
...
app.use('/uploads',express.static(path.join(rootDir,
   "uploads")));
